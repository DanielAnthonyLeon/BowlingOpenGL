...
// Returns the starting memory location for vertex normal data
const float * MeshConsolidator::getUVCoordsDataPtr() const {
	return &(m_uvCoordsData[0].x);
}
...
// Returns the total number of bytes of all the uv coordinates data
size_t MeshConsolidator::getNumUVCoordsBytes() const {
	return m_uvCoordsData.size() * sizeof(vec2);
}
